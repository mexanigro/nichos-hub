...
import { ROOT, SOY, conTemporal, correrLargo, git, repoTemporal, rojoVerdeTodas } from "./orden/verdad-04/_util.ts";
...
test("npm test corre tests/verdad-03-a.test.ts, -b, -c, -d, -e y -f (copias editables de los 12 tests de la orden, que importan ./orden/verdad-03/_util.ts; en la copia d, el test D2 comprueba existencia, import, package.json y git log sin volver a correr las copias de verdad-02, que npm test ya corre) y pasan 12/12; tests/orden/verdad-03/ no cambia", () => {
  ...
  // tests/orden/verdad-03/ no cambia: ningún commit posterior a su último rojo modifica, borra ni renombra lo que hay.
  assert.equal(git(ROOT, "log", "--format=%h %s", "--diff-filter=MDR", `${git(ROOT, "log", "-1", "--format=%H", "--diff-filter=A", "main", "--", "tests/orden/verdad-03/HOJA.md")}..main`, "--", "tests/orden/verdad-03/"), "", "tests/orden/verdad-03/ sigue congelada desde su último rojo");
});
