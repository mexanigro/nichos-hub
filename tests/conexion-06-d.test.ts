...
import { CONEXION_05, REPO, ROOT, conTemporal, cuentaTests, cuentaTestsDe, git, ordenesVivas, reproducirHead, rojoVerdeTodas } from "./orden/conexion-06/_comun.ts";
...
test("conexion-05 figura en tests/orden/APROBADAS.md con fecha 2026-09-23, T a41f93a y H 0c60811, y rojo-verde --todas en HEAD imprime «conexion-05 · retirada (aprobada 2026-09-23)» y no «orden conexion-05 ·»", () => {
  ...
  // --todas «en HEAD» sin correr estos mismos tests (que a su vez correrían --todas): HEAD se reproduce sin conexion-06/ y sin
  // ninguna orden viva, que --todas correría entera dentro de la suite.
  conTemporal((base) => {
    const { repo, ids } = reproducirHead(base, excluidas("conexion-06"));
