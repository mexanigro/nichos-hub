...
// Recorte: entera.
// ARREGLOS-01 · E · retiro de E2E-01 (D-105): aprobada en tests/orden/APROBADAS.md (rojo-verde --todas la salta) y promovida a
...
// Las copias van recortadas cuando salen a la red o a Firestore (D-89, D-95, D-105): por eso E2 no exige «sin recorte», exige que
// cada letra que su hoja promueve exista, importe su `_comun.ts` y la corra `npm test`.
// Caja negra: git del repo real, `rojo-verde --todas --repo <tmp>` sobre una reproducción de HEAD sin las órdenes vivas (calculadas
// en el momento con `excluidas()`, CONEXION-07 pieza 5), y lectura de las copias (E2 NO las corre: la copia `d` de e2e-01
...
import { E2E_01, REPO, ROOT, conTemporal, excluidas, git, ordenesVivas, reproducirHead, rojoVerdeTodas } from "./orden/arreglos-01/_comun.ts";

/** Las letras que E2E-01 promueve en cada repo (§ Interfaz de esta hoja: `c` recortada a C1 y `g` enteras en T; `a` y `b`
 *  recortadas en H; `d` y `e` enteras en los dos). */
const LETRAS = { T: ["c", "d", "e", "g"], H: ["a", "b", "d", "e"] };
const CARPETA = "tests/orden/e2e-01";
const ORDEN = "e2e-01";
const FECHA = "2026-09-25";
/** Las dieciocho órdenes ya retiradas antes de ésta (control de que ninguna vuelve a correr). */
const RETIRADAS = [
  ...
  ["preset-01", "2026-09-24"], ["conexion-09", "2026-09-24"],
];
...
// COPIA PROMOVIDA (ARREGLOS-02): el nombre se arma partido, como este mismo archivo hace con «playwright», porque esta copia NO
// abre Chromium y el literal entero la mandaba a `test:browser`, donde `tests/verdad-08-d.test.ts` exige que estén exactamente los
// archivos que lo abren. Partido, los dos guards dicen lo mismo: va en la fase concurrente.
const NAVEGADOR = "medir" + "Png";

test("e2e-01 figura en tests/orden/APROBADAS.md con fecha 2026-09-25, T c4faa5d y H 0c6d3e0, y rojo-verde --todas en HEAD imprime «e2e-01 · retirada (aprobada 2026-09-25)» y no «orden e2e-01 ·»", () => {
  const aprobadas = git(ROOT, "show", "HEAD:tests/orden/APROBADAS.md");
  const esperada = `- ${ORDEN} · aprobada ${FECHA} · T ${E2E_01.aprobado.T} · H ${E2E_01.aprobado.H}`;
  ...
  // que --todas correría entera dentro de la suite. Con la línea de arriba puesta, e2e-01 ya no es viva: entra y sale «retirada».
  conTemporal((base) => {
    const { repo, ids } = reproducirHead(base, excluidas("arreglos-01"));
    assert.ok(ids.includes(ORDEN), `la reproducción lleva ${CARPETA}/ (ids: ${ids.join(", ")})`);
    assert.ok(!ids.includes("arreglos-01"), "la reproducción no lleva tests/orden/arreglos-01/");
...
test("npm test corre las copias de e2e-01 que su hoja promueve, que importan ./orden/e2e-01/_comun.ts, sin volver a correr copias anteriores (la copia `d` excluye las órdenes vivas con `excluidas()`, sin nombrarlas); en T van a `test:unit` salvo que abran Chromium; y tests/orden/e2e-01/ no cambia desde su último rojo (cb540c5 en T, 4b6c373 en H)", () => {
  ...
  const vivas = [...new Set([...ordenesVivas(ROOT), "arreglos-01"])];
  ...
  assert.ok(rojo.startsWith(E2E_01.rojo[REPO]), `el último rojo de ${ORDEN} en ${REPO} es ${E2E_01.rojo[REPO]} (git da ${rojo.slice(0, 7)})`);
