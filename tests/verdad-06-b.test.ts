...
// el permiso .git/permitir-tests = 1 (VERDAD-07 D-30, antes HIGIENE_PERMITIR_TESTS=1); B3 falla cerrado (sin `command` string → ROTO), `command` vacío → 0, fuera de tests/orden → 0.
...
import { ROOT, bloqueContrato, candadoJson, conTemporal, correr, repoTemporal, type Salida } from "./orden/verdad-06/_util.ts";
...
test("candado.mjs con `tool_name` Bash o PowerShell y `tool_input.command` string veta (exit 2, «CANDADO · …») todo segmento del comando que nombre `tests/orden/<id>/` cuando `tests/orden/<id>/HOJA.md` está en la historia de main, salvo que el segmento sea una lectura reconocida (`cat`, `head`, `tail`, `sed -n`, `grep`, `rg`, `wc`, `ls`, `diff`, `cmp`, `sha256sum`, `node … --test`, `npx tsx --test`, `git diff|log|show|status|ls-files|blame|check-ignore`) o `HIGIENE_PERMITIR_TESTS` sea exactamente «1»; casos: `node -e \"require('fs').writeFileSync('tests/orden/<id>/c.test.ts','x')\"`, `sed -i s/a/b/ tests/orden/<id>/c.test.ts`, `cp x tests/orden/<id>/y`, `git checkout <sha> -- tests/orden/<id>/`, `echo x > tests/orden/<id>/HOJA.md` y `cd tests/orden/<id> && node -e …` → veto; `node --experimental-strip-types --test tests/orden/<id>/*.test.ts`, `git diff <sha> HEAD -- tests/orden/<id>/`, `cat tests/orden/<id>/HOJA.md`, `echo x > tests/orden/nueva-99/HOJA.md` (sin rojo) y `sed -i … tests/orden/APROBADAS.md` → pasan; Edit/Write siguen igual que hoy", () => {
  conTemporal((base) => {
    ...
    // Pasan: lecturas reconocidas, otra orden sin rojo, APROBADAS.md, y el permiso por archivo (VERDAD-07 D-30: <raíz>/.git/permitir-tests = 1;
    // la variable HIGIENE_PERMITIR_TESTS ya no abre).
    const permiso = resolve(dir, ".git", "permitir-tests");
    const conPermiso = (fn: () => Salida) => { writeFileSync(permiso, "1\n"); try { return fn(); } finally { rmSync(permiso, { force: true }); } };
    const permitidos: [string, Salida][] = [
      ...
      [".git/permitir-tests = 1", conPermiso(() => bash("echo x > tests/orden/vieja-01/HOJA.md"))],
    ];
    ...
    veto(bash("echo x > tests/orden/vieja-01/HOJA.md", { HIGIENE_PERMITIR_TESTS: "1" }), "HIGIENE_PERMITIR_TESTS=1 ya no abre (VERDAD-07 D-30)");
    ...
    veto(write(resolve(dir, "tests/orden/vieja-01/d.test.ts"), { HIGIENE_PERMITIR_TESTS: "1" }), "Write con HIGIENE_PERMITIR_TESTS=1 ya no abre (VERDAD-07 D-30)");
    pasa(conPermiso(() => write(resolve(dir, "tests/orden/vieja-01/d.test.ts"))), "Write con .git/permitir-tests = 1");
  });
});
