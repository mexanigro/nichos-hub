...
// VERDAD-03 D2 (2026-09-20): copia editable promovida a npm test (la orden está aprobada y retirada de rojo-verde --todas; el original
// en tests/orden/verdad-02/ queda congelado). A1 adaptado a «último commit que añade» HOJA.md y a la línea «rojos anteriores» (B1).
import { test } from "node:test";
import assert from "node:assert/strict";
import { spawnSync } from "node:child_process";
import { existsSync, readFileSync } from "node:fs";
import { join, resolve } from "node:path";
import { OTRO, ROOT, SOY, bloqueContrato, borrar, carpetaTemporal, correr, hojaMinima, repoTemporal, rojoVerde, testMinimo, type Repo } from "./orden/verdad-02/_util.ts";
...
test("El commit rojo de una orden es el último commit de main que añade tests/orden/<id>/HOJA.md (los anteriores salen como «rojos anteriores»); sin ese commit, rojo-verde --orden <id> sale con 2 y dice «sin commit rojo»", () => {
  ...
  assert.doesNotMatch(r2.stdout, /rojos anteriores/, "con un solo rojo no hay línea de rojos anteriores");
  borrar(base2);

  // «El ÚLTIMO commit que añade» (VERDAD-03 B1): corrección de A = revert del rojo + nuevo commit rojo; el rojo vigente es el segundo
  // y el primero queda a la vista en la última línea «rojos anteriores: <sha7>».
  const base3 = carpetaTemporal();
  const dos = repoTemporal(SOY, base3);
  const hoja = hojaMinima([["X1", "T+H", "frase uno"]]);
  const rojo1 = dos.commit("rojo 1", { [`${CARPETA}/HOJA.md`]: hoja, [`${CARPETA}/x.test.ts`]: testMinimo([["frase uno", "verde.txt"]]) });
  dos.git("rm", "-r", "-q", CARPETA);
  dos.git("commit", "-q", "-m", "revert del rojo 1");
  const rojo2 = dos.commit("rojo 2 (corregido)", { [`${CARPETA}/HOJA.md`]: hoja, [`${CARPETA}/x.test.ts`]: testMinimo([["frase uno", "verde.txt"]]) + "// corregido por A tras el revert\n" });
  dos.commit("verde", { "verde.txt": "verde\n" });
  const r3 = rojoVerde(ORDEN, dos.dir);
  assert.equal(r3.status, 0, `con el rojo corregido y verde debe salir 0 (salió ${r3.status})\n${r3.out}`);
  assert.ok(r3.stdout.includes(`rojo en ${sha7(rojo2)}`), `debe citar como rojo al último que añade HOJA.md ${sha7(rojo2)}\n${r3.stdout}`);
  assert.ok(!r3.stdout.includes(`rojo en ${sha7(rojo1)}`), `no debe citar como rojo al primero ${sha7(rojo1)}\n${r3.stdout}`);
  const ultima = r3.stdout.split(/\r?\n/).filter((l) => l.trim()).at(-1) ?? "";
