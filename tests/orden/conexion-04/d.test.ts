...
import { CONEXION_03, REPO, ROOT, conTemporal, correrLargo, cuentaTests, git, reproducirHead, rojoVerdeTodas } from "./_util.ts";
...
test("conexion-03 figura en tests/orden/APROBADAS.md con fecha 2026-09-22, T 4c44c0c y H 47d2908, y rojo-verde --todas en HEAD imprime «conexion-03 · retirada (aprobada 2026-09-22)» y no «orden conexion-03 ·»", () => {
  ...
  // --todas «en HEAD» sin correr estos mismos tests (que a su vez correrían --todas): HEAD se reproduce en un repo temporal sin conexion-04/.
  conTemporal((base) => {
    const { repo, ids } = reproducirHead(base, ["conexion-04"]);
