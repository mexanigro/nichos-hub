...
test("src/lib/palette.ts y src/lib/oklab.ts de H existen y son byte a byte iguales a los de T, y tests/paleta-paridad.test.ts de H lo exige en reposo (con una orden viva en cualquiera de los dos repos imprime «paridad de palette/ diferida: orden viva <id>» y pasa, como el guard de tools/, D-61); con `src/lib/oklab.ts` de H cambiado en un byte y sin órdenes vivas (probado con `HIGIENE_ROOTS` sobre dos temporales) el guard falla nombrando el archivo", () => {
  ...
  // Sin NODE_TEST_CONTEXT (hallazgo de B, 2026-09-23): este archivo lo corre `node --test` como hijo, con esa variable puesta; el
  // runner anidado la heredaría, avisaría «run() is being called recursively … skipping running files» y saldría 0 con stdout vacío,
  // así que los dos casos de abajo pasarían o fallarían sin cargar nunca el guard. Mismo campo que las demás llamadas anidadas.
  const correrGuard = (roots: [string, string]) => correr(["--experimental-strip-types", "--test", GUARD_PARIDAD], { env: { HIGIENE_ROOTS: `${roots[0]};${roots[1]}`, NODE_TEST_CONTEXT: undefined } });
