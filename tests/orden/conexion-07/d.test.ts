...
// Caja negra: git del repo real, `rojo-verde --todas --repo <tmp>` sobre una reproducción de HEAD sin conexion-07/, y lectura de las
// copias (D2 NO las corre: la copia `d` de conexion-06 reproduce HEAD, que lleva esta orden viva). Un mismo archivo en T y en H
// (cmp → 0): las copias y el conteo se eligen por REPO.
import { test } from "node:test";
import assert from "node:assert/strict";
import { existsSync, readFileSync } from "node:fs";
import { matchesGlob, resolve } from "node:path";
import { CONEXION_06, REPO, ROOT, conTemporal, cuentaTests, cuentaTestsDe, git, ordenesVivas, reproducirHead, rojoVerdeTodas } from "./_comun.ts";
...
test("conexion-06 figura en tests/orden/APROBADAS.md con fecha 2026-09-23, T b5b78f7 y H 07739a1, y rojo-verde --todas en HEAD imprime «conexion-06 · retirada (aprobada 2026-09-23)» y no «orden conexion-06 ·»", () => {
  ...
  // --todas «en HEAD» sin correr estos mismos tests (que a su vez correrían --todas): HEAD se reproduce sin conexion-07/.
  conTemporal((base) => {
    const { repo, ids } = reproducirHead(base, ["conexion-07"]);
...
test("npm test corre las copias tests/conexion-06-a.test.ts, -d, -e en H (9 tests) y tests/conexion-06-b.test.ts, -d, -e en T (5 tests), que importan ./orden/conexion-06/_comun.ts, sin recorte y sin volver a correr copias anteriores; en T van a `test:unit`; tests/orden/conexion-06/ no cambia desde su último rojo (014523d en T, 6ad11a4 en H)", () => {
  ...
  for (const c of copias) {
    const src = leer(c);
    const m = src.match(/reproducirHead\([^,]+,\s*\[([^\]]*)\]/);
    if (!m) continue;
