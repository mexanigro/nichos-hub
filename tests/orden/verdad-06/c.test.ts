...
import { ROOT, SOY, clonar, conTemporal, gitCrudo, sha256 } from "./_util.ts";
...
test("para cada archivo rastreado, `git hash-object --no-filters <archivo>` es igual al sha del índice (`git ls-files -s`): cero diferencias (disco = blob byte a byte); el test lista los que difieran", () => {
