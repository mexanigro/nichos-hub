...
import { NOMBRE, ROOT, SOY, conTemporal, correr, fuenteFixture, leerRegistro, norm, repoConRojo, reproducirHead, rojoVerde, rojoVerdeTodas, type Registro } from "./_util.ts";
...
test("`.githooks/pre-push` sigue corriendo `rojo-verde --todas` y `tests/contrato-hooks.test.ts` pasa sin cambios; el tiempo de `--todas` en HEAD con las seis órdenes retiradas y verdad-07 viva se imprime al final («tiempo: N s»)", (t) => {
  ...
  conTemporal((base) => {
    ...
    assert.deepEqual(retiradas, [...ids].sort(), `las seis órdenes de HEAD (${ids.join(", ")}) salen «retirada» (exit ${r.status}):\n${r.out.slice(-2500)}`);
    assert.equal(ids.length, 6, `seis órdenes en HEAD sin contar verdad-07: ${ids.join(", ")}`);
    assert.equal(r.status, 0, `--todas con las seis retiradas sale 0 (salió ${r.status})\n${r.out.slice(-2000)}`);
  });
  assert.ok(existsSync(resolve(ROOT, "tests/orden/verdad-07/HOJA.md")), "verdad-07 viva: su HOJA.md existe");
  assert.doesNotMatch(readFileSync(resolve(ROOT, "tests/orden/APROBADAS.md"), "utf8"), /^- verdad-07 /m, "verdad-07 viva: no figura en APROBADAS.md");
});
