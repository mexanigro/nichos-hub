...
import { CONEXION_04, REPO, ROOT, cuentaTests, git } from "./orden/verdad-08/_util.ts";
...
test("conexion-04 figura en tests/orden/APROBADAS.md con fecha 2026-09-22, T 4a22d7b y H f8bd9bb, y rojo-verde --todas en HEAD imprime «conexion-04 · retirada (aprobada 2026-09-22)» y no «orden conexion-04 ·»", () => {
  ...
  // VERDAD-09 C2: la copia no reproduce HEAD ni corre `--todas` (HEAD lleva órdenes vivas y esta copia corre dentro de `npm test`:
  // sería recursión y volvería a correr las copias anteriores). Lo que hace que rojo-verde la salte es la línea de APROBADAS.md, que
  // es lo que se comprueba aquí, junto con las nueve retiradas anteriores (control: ninguna vuelve a correr).
  for (const [id, fecha] of RETIRADAS) {
    assert.ok(aprobadas.split(/\r?\n/).some((l) => l.trim().startsWith(`- ${id} · aprobada ${fecha} ·`)), `control: ${id} sigue retirada (aprobada ${fecha}):\n${aprobadas}`);
  }
  assert.ok(existsSync(resolve(ROOT, `${CARPETA}/HOJA.md`)), `${CARPETA}/ sigue en el árbol, congelada`);
});

test("npm test corre las copias tests/conexion-04-a.test.ts, -b, -d, -e en H (8 tests) y tests/conexion-04-b.test.ts, -d, -e en T (5 tests) (en T por `test:unit`, porque `test` encadena las fases, D-57), que importan ./orden/conexion-04/_util.ts, sin recorte (cada copia lleva los mismos `test(` que su original en la carpeta congelada), y la copia `d` no importa `correrLargo`: no vuelve a correr las copias anteriores (B2); tests/orden/conexion-04/ no cambia desde su último rojo (47490a2 en T, 00b4fe0 en H)", () => {
  ...
  // npm test los corre: los scripts de package.json los nombran (T: `test` encadena las fases `test:unit` y `test:browser`, D-57) o los cubren con un glob (H).
  const scripts = JSON.parse(readFileSync(resolve(ROOT, "package.json"), "utf8")).scripts ?? {};
  const script = ["test", "test:unit", "test:browser"].map((s) => String(scripts[s] ?? "")).join(" ");
