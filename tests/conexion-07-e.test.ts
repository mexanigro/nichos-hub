...
// commit aprobado de CONEXION-06 (T b5b78f7 / H 07739a1): esa orden no toca hooks.
...
// Lee disco (el otro repo por ruta fija) y compara el bloque línea a línea, sin CR, por `git show`. Un mismo archivo en T y en H
// (cmp → 0).
import { test } from "node:test";
import assert from "node:assert/strict";
import { readFileSync } from "node:fs";
import { join } from "node:path";
import { CONEXION_06, RAIZ_H, RAIZ_T, bloqueDe, git, seccionDeTexto } from "./orden/conexion-07/_comun.ts";

const lineas = (bloque: string) => bloque.split(/\r?\n/).map((l) => l.replace(/\s+$/, ""));
/** Lo que el párrafo de esa orden tiene que nombrar: la línea base nueva y los dos guards de T (D-77). */
