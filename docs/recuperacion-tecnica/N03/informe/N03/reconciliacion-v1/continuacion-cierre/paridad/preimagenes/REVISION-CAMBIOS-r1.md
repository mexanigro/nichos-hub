...
- MATERIAL: api/index.ts:1 importa checkout-handler sin extensión .js. Las demás factories del runtime usan .js y el contrato técnico existente de api-parity explica que Vercel ESM requiere esa forma. checkout.cjs inyecta la factory transpilada y no carga ese import; GREEN no acredita su resolución. Corrección mínima propuesta a root: ../src/lib/api/checkout-handler.js. No cambiar import server por este hallazgo. Pendiente al corte de esta revisión; no se ejecutó proveedor ni Vercel para afirmar un error remoto observado.
