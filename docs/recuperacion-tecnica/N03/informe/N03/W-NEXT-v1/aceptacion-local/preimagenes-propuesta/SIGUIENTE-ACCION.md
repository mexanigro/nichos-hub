...
Obligación: que los patches emitidos por **este endpoint** actualicen la representación que consumen wizard/configuración, preservando los datos no enviados. Incluye sus escrituras config y hub_clients para no dejar la misma causa a medias;conservar estados,auth/tenant,límite,historial y política de efectos.
...
2. El lector real recupera los valores enviados,por ejemplo modo,nombre y contacto;los campos no enviados,imágenes y campos hermanos permanecen idénticos. Arrays y valores especiales de Firestore mantienen la semántica actual que corresponda.
3. La prueba sigue el recorrido de escritura/lectura pertinente con semántica del SDK instalado, sin depender de un mock que divida puntos incorrectamente. Restaurar la escritura literal en copia debe volver a RED por el campo que no cambia,no por fallos de arranque/importación.
