...
import { Scissors, Sparkles, Paintbrush, Hand, Coffee, Hammer, MoreHorizontal } from "lucide-react";
...
const NICHE_LABELS: Record<string, Record<string, string>> = {
  ...
  cafeteria: { en: "Café", es: "Cafetería", ru: "Кафе", he: "בית קפה" },
  remodelaciones: { en: "Remodeling", es: "Remodelaciones", ru: "Ремонт", he: "שיפוצים" },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {NICHES.slice(0, 6).map(({ id, icon: Icon }) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-2.5 rounded-[var(--l-radius-sm)] border p-4 transition-all duration-200 active:scale-[0.97] ${
                selected
            ...
            >
              ...
              />
              <span className="text-[0.82rem] font-medium text-[var(--l-text)]">
      ...
      <div className="mt-3 flex justify-center">
        {(() => {
          const { id, icon: Icon } = NICHES[6];
          const selected = data.niche === id;
          return (
            <button
              onClick={() => update({ niche: id })}
              aria-pressed={selected}
              className={`flex w-full max-w-[calc(33.333%-8px)] flex-col items-center gap-2.5 rounded-[var(--l-radius-sm)] border p-4 transition-all duration-200 active:scale-[0.97] ${
                selected
                  ? "border-[var(--l-accent)] bg-[var(--l-accent-muted)]"
                  : "border-[var(--l-border)] hover:border-[var(--l-accent)]"
              }`}
