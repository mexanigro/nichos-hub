import { NextRequest, NextResponse } from "next/server";
import { db } from "@/lib/firebase-admin";
import { FieldValue } from "firebase-admin/firestore";
import { CURRENCY, PLAN_ID } from "@/lib/pricing";
import { resolveClientCharge } from "@/lib/charges";
import { isRateLimited } from "@/lib/rate-limit";

export async function POST(req: NextRequest) {
  const ip = req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "unknown";
  if (isRateLimited(ip, "contract", 5, 60_000)) {
    return NextResponse.json({ error: "Demasiadas solicitudes" }, { status: 429 });
  }

  let body: Record<string, unknown>;
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ error: "Invalid JSON" }, { status: 400 });
  }

  const { clientId, clientDocId, contractVersion } = body as {
    clientId?: string;
    clientDocId?: string;
    contractVersion?: string;
  };

  if (!clientId || !clientDocId) {
    return NextResponse.json(
      { error: "clientId y clientDocId son requeridos" },
      { status: 400 },
    );
  }

  // Verify client exists
  const clientDoc = await db.collection("hub_clients").doc(clientDocId).get();
  if (!clientDoc.exists) {
    return NextResponse.json({ error: "Cliente no encontrado" }, { status: 404 });
  }

  const clientData = clientDoc.data()!;
  if (clientData.clientId !== clientId) {
    return NextResponse.json({ error: "clientId no coincide" }, { status: 400 });
  }

  // Venta en persona (P-3): un cliente dado de alta desde el hub es `demo` o `active` y debe poder firmar y pagar.
  // Sólo se bloquea lo que está fuera de servicio.
  const blockedStatuses = new Set(["suspended", "archived"]);
  if (clientData.status && blockedStatuses.has(clientData.status)) {
    return NextResponse.json({ error: "Estado del cliente no permite crear contrato" }, { status: 403 });
  }

  // Alta (setupAmount negociado o 1500) si nunca pagó el alta; si no, cuota 250. Mismo criterio que create-payment.
  const charge = await resolveClientCharge(clientId, clientData);
  const type = charge.kind === "initial" ? "initial" : "recurring";
  const amount = charge.amount;

  const now = FieldValue.serverTimestamp();
  const today = new Date();
  const nextBilling = new Date(today);
  nextBilling.setMonth(nextBilling.getMonth() + 1);

  let docRef;
  try {
    docRef = await db.collection("hub_payments").add({
      clientId,
      clientDocId,
      businessName: clientData.businessName || "",
      plan: PLAN_ID,
      amount,
      currency: CURRENCY,
      type,
      status: "pending",
      billingDate: today,
      nextBillingDate: nextBilling,
      cardLastFour: null,
      failureReason: null,
      cardcomTransactionId: null,
      contractAccepted: true,
      contractAcceptedAt: now,
      contractVersion: contractVersion || "2.0",
      contractIp: ip,
      createdAt: now,
      updatedAt: now,
    });
  } catch (err) {
    console.error("[contract] Firestore write failed:", err);
    return NextResponse.json({ error: "Error al registrar el contrato" }, { status: 500 });
  }

  return NextResponse.json({
    id: docRef.id,
    type,
    amount,
  }, { status: 201 });
}
