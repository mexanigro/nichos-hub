...
const EN = `SERVICE AGREEMENT — WEBSITE, CRM & WHATSAPP AGENT
₪770/month — All-inclusive
...
AND WHEREAS the Client wishes to subscribe to the Service (website + CRM + WhatsApp agent) for online visibility, digitalisation, and business management;
...
2.1. Monthly subscription: ₪770/month.
...
const HE = `הסכם מתן שירות – אתר, CRM וסוכן WhatsApp
₪770/חודש — הכל כלול
...
והואיל והמזמין מבקש להצטרף לשירות (אתר + CRM + סוכן WhatsApp);
...
2.1. דמי מנוי חודשיים: ₪770 לחודש.
...
const RU = `ДОГОВОР НА ОКАЗАНИЕ УСЛУГ — САЙТ, CRM И АГЕНТ WHATSAPP
₪770/мес — Всё включено
...
И принимая во внимание, что Заказчик желает подключиться к Сервису (сайт + CRM + агент WhatsApp);
...
2.1. Ежемесячная абонентская плата: ₪770 в месяц.
...
const ES = `ACUERDO DE SERVICIOS — SITIO WEB, CRM Y AGENTE WHATSAPP
₪770/mes — Todo incluido
...
Y considerando que el Cliente desea suscribirse al Servicio (sitio web + CRM + agente WhatsApp) para visibilidad en internet, digitalización y gestión operativa de su negocio;
...
2.1. Suscripción mensual: ₪770/mes.
