...
The reason should be one short sentence in Spanish.`,
    ...
    const validCategories: MessageCategory[] = ["maintenance", "support", "conversation"];
    const category = validCategories.includes(parsed.category) ? parsed.category : "conversation";

    return { category, reason: parsed.reason || "" };
  } catch (err) {
    console.error("[classify] failed:", err instanceof Error ? err.message : err);
    return { category: "conversation", reason: "Clasificacion no disponible" };
  }
}
